...
class TestResolveSafeCwd:
    ...
    def test_returns_root_when_only_root_exists(self, monkeypatch):
        ...
        itself is still a valid recovery target; don't skip it when dirname
        reaches its platform-specific fixed point."""
        ...
        monkeypatch.setattr(
            os.path,
            "isdir",
            lambda path: os.path.normcase(os.path.normpath(path)) == normalized_root,
        )
